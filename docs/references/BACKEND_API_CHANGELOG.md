...
| 2026-07-18 | user / follow / message | Java 已实现 me summary/stand/contents/favorites/comments、公开 profile/contents/followings/followers、用户 follow/unfollow 和通用 USER/TEAM/PLAYER toggle；没有我的点赞列表及任何消息/通知接口 | F07 我的、用户主页、关注、消息 | 无 | 真实接口已适配；缺失能力显示正式不可用 | F07 双账号 smoke | following Feed 按服务端关注关系查询；后端未修改 |
| 2026-07-19 | auth / admin health | 登录返回 accessToken/tokenType/expiresIn/user，me 返回 roleType=ADMIN/USER；`/api/admin/**` 使用 hasRole(ADMIN)，无 refresh/logout | F08 Vue 登录、守卫、后台框架 | 无 | 候选 Token 经 me 校验后写 sessionStorage；USER 进入 403 | F08 管理员/USER smoke | 管理员凭据仅复用后端既有 seed smoke 机制；后端未修改 |
